...
// Copy over the current build of @remix-run/shared-internals and execute 'remix init' in the test project's context.
// Certain bugs are only tested for when using this approach (vs directly calling remix-shared-internals/cli/run).
const execRemixInitInProject = (projectDir: string) => {
  execSync(`yarn install`, {
    stdio: "inherit",
  });

  // overwrite installed @remix-run/shared-internals with the one we just built
  fse.copySync(
    path.join(
      __dirname,
      "../../../",
    ...
    path.join(projectDir, "node_modules", "@remix-run/shared-internals")
  );
